...
class CRFTagger(object):
    def __init__(self, config):
        ...
        self.x = tf.placeholder(tf.float32, [None, self.max_step, self.n_inputs], name='input') 
        self.y = tf.placeholder(tf.float32, [None, self.max_step, self.n_classes], name='label') 
...
#        self.__prediction = tf.nn.softmax(mask_logit) # prediction = mask_logit
    ...
    def RNN(self, X, weights, biases,seq_length,dropout):
        ...
        outputs, state = tf.nn.dynamic_rnn(cell, inputs,sequence_length=seq_length,dtype=tf.float32)
